...
void path_planner()
{	int startingno = 0 , curr_x , curr_y;
	curr_y = y_coordinate;
	curr_x =x_coordinate;
	startingno = map_array[x_coordinate][y_coordinate];
	for(int l = startingno-1; l<0 ; l=l-1)
	{
		...
		for (int i = 0; i < 9; i++)
		{
			for (int j = 0; i < 9; i++)
			{
				if(map_array[i][j] == l)
				{
					...
					{
						if(dir_array[curr_x][curr_y][3] == dir_array[i][j][1])
						{
							switch(dir)
							{	
								case +2: turn_left();
							...
							//bot_move new function
						}
					}
					if (curr_x == i && curr_y > j)
					{
						if(dir_array[curr_x][curr_y][2] == dir_array[i][j][0])
						{
							switch(dir)
							{	
								case +1:turn_right(); 
					...
					if (curr_x < i && curr_y == j)
					{
						if(dir_array[curr_x][curr_y][1] == dir_array[i][j][3])
						{
							switch(dir)
							{	
								case +1:
					...
					if (curr_x == i && curr_y < j)
					{
						if(dir_array[curr_x][curr_y][0] == dir_array[i][j][2])
						{
							switch(dir)
							{	
								case +1: turn_left();
...
void build_map(int start_x , int start_y , int destn_x , int destn_y)    //function to be called after dest is indicated on map array
{
	for (int i = 0; i < 4; ++i)
	{
		for (int j = 0; j < 4; ++j)
